# Master Professional Profile

This document is the single source of truth for the professional identity used across the GitHub portfolio, resume, LinkedIn profile, and future job applications.

## Professional Identity

**Mohammed A.M. Aldirawi**

**AI Trainer | AI Data Annotation | Prompt Engineering | Business & Finance Domain Expert**

## Professional Positioning

Accounting and business professional transitioning into AI training, evaluation, annotation, and prompt engineering. Combines a Bachelor's degree in Accounting and professional experience in accounting, auditing, financial analysis, and business operations with practical, self-directed AI-training work.

### Core Value Proposition

**Business & Finance domain expertise + structured AI evaluation discipline.**

The professional story should consistently show that the candidate brings established accounting/business knowledge and applies it to practical AI evaluation, annotation, prompt engineering, factuality checking, and quality assurance.

## AI Capability Areas

- AI Training
- AI Response Evaluation
- AI Data Annotation
- Prompt Engineering
- Instruction Following Evaluation
- Factuality & Hallucination Detection
- Quality Assurance
- Error Analysis
- Data Labeling
- Text Classification
- Structured Data Review
- Response Improvement

## Business & Finance Domain Areas

- Accounting
- Financial Analysis
- Auditing
- Financial Management
- Business Management
- Economics
- Digital Economy
- Financial Markets
- Banking & Finance
- Marketing
- E-commerce

## Education

**Bachelor's Degree in Accounting**  
Islamic University of Gaza, Palestine  
2009–2013

**Academic Foundation:** Accounting, financial analysis, business, and related economic disciplines.

## Professional Experience — Consistency Rules

The resume and LinkedIn should describe the candidate's real accounting/business experience accurately and separately from the self-directed AI portfolio.

- **Teaching Assistant — Islamic University of Gaza, Palestine | 2014–2017**
  - Teaching accounting subjects and supporting accounting education.
- **Kaanan Company for Auditing, Palestine | 2017–2019**
  - Accounting/auditing work including invoice entry in Excel, account review, and tax-related work.
- **Tarbush Restaurant, Malaysia | 2019–2025**
  - Accounting/business operations including inventory movement, sales invoices, manufacturing quantities, sales comparison, and investigation of variances.
- **Al-Baraka / Berkat Madinah, Malaysia**
  - Purchases/accounting role; use the finalized resume wording and confirmed employment period consistently across resume and LinkedIn.

> Do not present any of the above as AI employment. AI capability should be represented through the self-directed portfolio, training, courses, and practical exercises.

## Training & Courses

Completed/self-directed AI learning should be listed consistently with the resume and LinkedIn, while distinguishing course completion from possession of a formal certificate where applicable.

Current completed Coursera learning includes:

- Evaluating Large Language Model Outputs: A Practical Guide
- Prompt Engineering for New Professionals
- Introduction to Generative AI

Additional completed courses should be added using their exact official course titles and completion status.

## Languages

- Arabic — Native
- English — Excellent
- Malay — Basic

## Portfolio Evidence

The GitHub portfolio is the evidence layer for the AI career transition. It contains 12 practical projects plus structured evidence artifacts covering evaluation, prompt engineering, annotation, business/finance AI training, instruction following, bias/fairness, hallucination detection, financial analysis, QA, workflow design, and data quality assurance.

All portfolio exercises must remain clearly identified as **simulated/self-directed** unless a future project is based on genuine professional or client work.

## Recruiter Message

A recruiter should understand the candidate in one sentence:

> **Accounting and business professional with strong finance-domain expertise, building practical AI training, evaluation, annotation, and prompt-engineering capability and demonstrating that capability through a structured portfolio.**

## Consistency Rules Across Platforms

1. Use the exact professional title: **AI Trainer | AI Data Annotation | Prompt Engineering | Business & Finance Domain Expert**.
2. Keep education as **Bachelor's Degree in Accounting, Islamic University of Gaza, 2009–2013**.
3. Keep **Malay — Basic** consistently across Resume, LinkedIn, and GitHub.
4. Keep real employment experience separate from self-directed AI portfolio work.
5. Use the same AI capability categories across resume, LinkedIn, and GitHub.
6. Link the GitHub portfolio from the resume and LinkedIn.
7. Feature **Business & Finance AI Training** and **AI Response Evaluation** as the strongest portfolio examples.
8. Surface selected portfolio evidence in the resume so Recruiters can move directly from resume to GitHub evidence.
9. Never claim paid AI employment, client work, or professional AI experience unless it is factually true.
10. Keep the portfolio's transparency statement consistent: practical capability is demonstrated through self-directed/simulated exercises.
